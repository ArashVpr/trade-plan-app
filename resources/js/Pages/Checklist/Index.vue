    ...
    <div class="max-w-5xl mx-auto p-6 bg-gray-50 min-h-screen">
        ...
        <div class="bg-white p-6 rounded-lg shadow-md">
            ...
            <div class="mt-6 flex justify-center" v-if="checklists.data.length > 0">
                <Link v-for="link in checklists.links" :key="link.label" v-html="link.label" :disabled="!link.url"
                method="get"
                as="button"
                    :class="[
                        'px-4 py-2 mx-1 rounded-md',
                        link.active ? 'bg-blue-900 text-white  hover:cursor-pointer' :
                            link.url ? 'bg-gray-200 text-gray-700 hover:bg-gray-300 hover:cursor-pointer' :
